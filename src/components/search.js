...
export default function SearchForMedia(props) {
   ...
   const handleSearch = async () => {
      ...
      try {
         const searchResults = await fetchOrDeleteFromApi(
...
function SearchResultsContainer(props) {
   ...
   return (
      ...
      >
         ...
         ) : !isSearchComplete ? null : searchResults.length === 0 ? (
...
function ResultCardOMDB(props) {
   //using OMDB api properties
   const {
      media,
      isSpecificList,
      list,
   ...
   const mediaTypeResult =
      media.q === "TV series" || media.q === "TV mini-series" ? "tv" : "movies"
   ...
   return (
      <div className={style.resultCard}>
         ...
         <div className={style.actionContainer}>
            {isSpecificList ? (
               <SpecificListAction imdbID={media.id} list={list} />
            ) : (
...
function NonSpecificListAction(props) {
   const { imdbID, refreshList, mediaType, tvListNames, movieListNames } = props
   ...
   return (
      <div className={style.nonSpecificAction}>
         <button onClick={handleActivityOpen} className={style.addToListButton}>
...
export function AddToList(props) {
   ...
   const handleSelection = (list, listCategory) => {
      handleAddToList(listCategory, list._id).catch((err) => console.log(err))
   }
   return (
      <div className={style.allListsContainer}>
         <WaitForServer wait={wait} waitText="Saving to your list" />
         <div>
            <div>
               <StreamingSourceFieldset
            ...
            <div className="errorDiv">
               {formik.values.streamingSource === "selectOne" && "Required"}
            </div>
         </div>
         <ListSelector
            showTv={showTv}
