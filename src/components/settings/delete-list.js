...
export default function DeleteList(props) {
   ...
   const handleDeleteList = async (listCategory, listName, listID) => {
      if (
         !window.confirm(`Are you sure you want to delete '${listName}' list?`)
      )
         return
   ...
   const handleSelection = (list, listCategory) => {
      handleDeleteList(listCategory, list.name, list._id).catch((err) =>
         console.log(err)
      )
   }
   return (
      <div className={style.deleteContainer}>
         ...
         />
         <WaitForServer wait={wait} waitText="Deleting List" />
      </div>
   )
}
