...
export function NewList(props) {
   ...
   return (
      <React.Fragment>
         ...
         <WaitForServer wait={wait} waitText="Creating New List" />
      </React.Fragment>
   )
}
export function EditList(props) {
   ...
   const { handleActivityClose, refreshList, tvListNames, movieListNames } =
      props
      ...
      onSubmit: async (values) => {
         ...
         try {
            ...
            if (err === "not-found") {
               refreshList(values.typeOfList)
               window.alert(
                  "Unsuccessful - List could have been deleted on a different device"
               )
               return
            }
            window.alert(`Unsuccessful - ${err}`)
   ...
   return (
      <div className={style.editContainer}>
         ...
         {!showLists && <ListForm formik={formik} disableSelectType={true} />}
...
function ListForm(props) {
   ...
   return (
      <form onSubmit={formik.handleSubmit} className={style.newListForm}>
         <fieldset>
            <label htmlFor="typeOfList">List Type</label>
            <select
               name="typeOfList"
               id="typeOfList"
         ...
         <div className="errorDiv">{formik.errors.listName}</div>
         <fieldset>
