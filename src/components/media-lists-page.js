   ...
   return (
      <div className={listContainerStyle.mediaListPage}>
         <div className={listContainerStyle.allListsContainer}>
            ...
            {allLists &&
               sortLists(allLists, listPref, mediaPref).map((list) => (
                  <ListContainer
                     key={list.name}
                     list={list}
            ...
            <div className={listContainerStyle.completedListContainer}>
               {completedList &&
                  ...
                  completedList.mediaInstants &&
                  completedList.mediaInstants.length !== 0 && (
                     <ListContainer
                        list={
                           sortLists([completedList], listPref, mediaPref)[0]
                        }
                        listCategory={listCategory}
                        refreshList={refreshList}
                        expandByDefault={false}
         ...
         >
            {!list.mediaInstants || list.mediaInstants.length === 0 ? (
               <div className={listContainerStyle.emptyLists}>
                  ...
                  <p className={listContainerStyle.emptyListText}>
                     To add a {listCategory === "towatch" ? "movie" : "tv show"}
                     ,
                  </p>
                  <p className={listContainerStyle.emptyListText}>
...
function ListDetails(props) {
   ...
   return (
      <div className={listContainerStyle.listDetails}>
         <h1 className={listContainerStyle.listName}>
            {list.name.toLocaleUpperCase()}
         </h1>
         <h2 className={listContainerStyle.listDescription}>
            {list.description}
         </h2>
      </div>
   )
   ...
   return (
      <div className={userMediaStyle.userMediaCard}>
         ...
         <div className={userMediaStyle.mediaDetails}>
            ...
            >
               <MoreInfoCard media={media} userMedia={userMedia} />
            </PopUpActivity>
         </div>
      </div>
   )
...
function MoreInfoCard(props) {
   ...
   const { isWatched, toWatchNotes, reviewNotes, userRating, imdbID } =
      userMedia
      ...
      <div className={userMediaStyle.moreInfoContainer}>
         <section className={userMediaStyle.posterAndMainInfoContainer}>
            ...
            <div className={userMediaStyle.mainInfoContainer}>
               <h1 className={userMediaStyle.mediaTitle}>{title}</h1>
               <p className={userMediaStyle.runtime}>
                  ({runtime ? runtime : "-"} min)
               </p>
               <p className={userMediaStyle.genre}>
                  {genre && genre.join(", ")}
               </p>
               <p className={userMediaStyle.releaseDate}>
                  {runYears && totalSeasons
                     ? `${runYears} (${totalSeasons} season${
                          totalSeasons > 1 ? "s" : ""
                       })`
                     : releaseDate}
               </p>
               <p className={userMediaStyle.imdbRating}>
                  ...
                  <span className={userMediaStyle.imdbRatingSpan}>
                     {imdbRating || "-"}
                  </span>
                  /10
               </p>
         ...
         <section className={userMediaStyle.otherInfoContainer}>
            ...
            >
               <p className={userMediaStyle.actors}>
                  {actors && actors.join(", ")}
               </p>
            </CollapsibleCard>
            <CollapsibleCard
               ...
               collapseButton={<CgArrowsExpandUpLeft />}
            >
               <p className={userMediaStyle.toWatchNotes}>
                  {toWatchNotes || "-"}
               </p>
            </CollapsibleCard>
            <CollapsibleCard
...
function MovieActionsMenuContainer(props) {
   ...
   const handleCloseAddToOtherList = (action) => {
      ...
      if (action === "move") {
         if (useCompleted) return handleMarkInComplete()
   ...
   const handleRemoveFromList = () => {
      ...
      fetchOrDeleteFromApi(
         `lists/${listCategory}/${list._id}/${imdbID}`,
         "delete"
      )
   ...
   return (
      <>
         <WaitForServer wait={wait} waitText={"Removing movie from list"} />
         <div className={userMediaStyle.menuContainer}>
            <CollapsibleCard
               collapseButton={
                  <HiDotsHorizontal className={userMediaStyle.menuButton} />
               }
               skipAllStyling={true}
               isCollapsed={true}
            ...
            >
               <div className={userMediaStyle.buttonContainer}>
                  ...
                  <button onClick={handleOpenAddToOtherList}>
                     Add to List
                  </button>
...
function UserNotesAndRating(props) {
   return (
      <div>
         <p>
            <span className={userMediaStyle.userRating}>
               {props.userRating === 0 ? "-" : props.userRating}
            </span>
            /10
         </p>
         <p className={userMediaStyle.reviewNotes}>
            {props.reviewNotes || "-"}
         </p>
      </div>
   )
