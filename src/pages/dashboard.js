...
export default function Dashboard(props) {
   ...
   useEffect(() => {
      if (!user) return
      refreshListNames().catch((err) => console.log(err))
   ...
   useEffect(() => {
      ...
      async function initialGetLists() {
         const movieLists = await fetchOrDeleteFromApi(
            `lists/towatch`,
            "get"
         ).catch((err) => console.log(err))
         const tvLists = await fetchOrDeleteFromApi(
            `lists/towatchtv`,
            "get"
         ).catch((err) => console.log(err))
      ...
      initialGetLists().catch((err) => console.log(err))
   }, [user])
   ...
   const handleUpdatedList = (listCategory) => {
      getLists(listCategory).then(refreshListNames).then(updateCompletedList)
   }
   if (!user)
      return (
         <>
         ...
         <div
            ...
            className={
               currentPage === "settings"
                  ? dashboardStyle.compensateMobileBottom
                  : undefined
            }
         />
         <PageSelector currentPage={currentPage} handlePages={handlePages} />
      </>
   )
}
...
function PageSelector(props) {
   ...
   useEffect(() => {
      ...
      const checkRaise = () =>
         setRaiseComponent(window.innerHeight > defaultHeight)
   ...
   const handlePageChange = (newPage, title) => {
      return () => {
         props.handlePages(newPage, title)
      }
   }
   const pages = [
      {
   ...
   return (
      <div className={selectorStyle.selectorWrapper}>
         <div className={selectorStyle.selectorContainer}>
            {pages.map((page) => (
               <button
                  ...
                  className={`${selectorStyle.selectorButton} 
                                ${
         ...
         <div
            className={`${selectorStyle.raiseOnFullscreen} ${
               raiseComponent ? selectorStyle.raise : ""
            }`}
         />
      </div>
   )
